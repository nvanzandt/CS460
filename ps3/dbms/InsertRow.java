...
public class InsertRow {
    ...
    public void marshall() throws IOException {
        ...
        // check if primary key and add to keyBuffer 

        if (this.table.primaryKeyColumn() != null)
        {
            for (int i = 0; i < this.columnVals.length; i++) {
                if (this.table.getColumn(i).isPrimaryKey()) { 

                    // if the key is of type int write the int
                    if (this.table.getColumn(i).getType() == 0){ 
                        this.keyBuffer.writeInt((int) this.columnVals[i]);
                    }
                    // if the key is of type double write the double
                    else if (this.table.getColumn(i).getType() == 1){ 
                        this.keyBuffer.writeDouble((double) this.columnVals[i]);
                    }
                    // if the key is of type char or varchar write the string
                    else { 
            ...
            // if the key is of type int write the int
            if (this.table.getColumn(0).getType() == 0){ 
                this.keyBuffer.writeInt((int) this.columnVals[0]);
            }
            // if the key is of type double write the double
            else if (this.table.getColumn(0).getType() == 1){ 
                this.keyBuffer.writeDouble((double) this.columnVals[0]);
            }
            // if the key is of type char or varchar write the string
            else { 
                this.keyBuffer.writeBytes((String) this.columnVals[0]);
            }
        }
        
        
        
        // add offsets to valueBuffer then add columnVals (no nulls)
        for (int i = 0; i < offsets.length; i++) {
            this.valueBuffer.writeShort(this.offsets[i]);
        }
        for (int i = 0; i < this.columnVals.length; i++) { 
            if (this.table.getColumn(i).isPrimaryKey() == false && this.columnVals[i] != null){ 
                // if the key is of type int write the int
                if (this.table.getColumn(i).getType() == 0){ 
                ...
                // if the key is of type double write the double
                    ...
                    this.valueBuffer.writeDouble((double) this.columnVals[i]);
                }
                // if the key is of type char or varchar write the string
                else { 
                    this.valueBuffer.writeBytes((String) this.columnVals[i]);
                }
            }
        }
    }
    
    // fill in the offsets array that proceeds columnVals
    public void fillOffsets() { 
        int offsetSum = this.offsets.length * 2;
        boolean offsetLengthFilled = false;

        for (int i = 0; i < this.offsets.length -1; i++) {
            // if primary key add -2
            if (this.table.getColumn(i).isPrimaryKey()) { 
                this.offsets[i] = -2;
            }
            else { 
                 // if columnVal is null add -1
                if (this.columnVals[i] == null) { 
                    this.offsets[i] = -1;
                }
                // add offsets array length to offsets array if it hasn't been added yet
                else if (offsetLengthFilled == false) { 
                    this.offsets[i] = offsetSum; 
                    offsetSum += getBytesLength(i);
                    offsetLengthFilled = true;
                }
                // add offsetSum to offsets array and increase offsetSum
                else {
                    this.offsets[i] = offsetSum;
                    offsetSum += getBytesLength(i);
                }
        ...
        // fill in last offset with offsetSum
        this.offsets[this.offsets.length - 1] = offsetSum;
    }


    // return the length of the bytes of the columnVal
    public int getBytesLength(int i) { 
        if (this.columnVals[i] == null) { 
            return 0;
        }
        else if (this.table.getColumn(i).getType() == 3) { 
            return ((String) this.columnVals[i]).length();
        }
        else { 
            return this.table.getColumn(i).getLength();
        }
    }
