...
public class SelectStatement extends SQLStatement {
    ...
    public void execute() throws DatabaseException, DeadlockException {
        ...
        try {
            ...
            if (this.numTables() > 1) {
                throw new Exception("SELECT commands with more than one table in the FROM clause are not supported");
            }
            if (this.numColumns() > 0) {
                throw new Exception("SELECT commands with column names are not supported");
            }
