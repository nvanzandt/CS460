...
public class TableIterator {
    ...
    public Object getColumnVal(int colIndex) {
        ...
        if (col.isPrimaryKey()) {
            if (colType == 0) { 
                return keyIn.readNextInt();
            }
            else if (colType == 1) {
                return keyIn.readNextDouble();
            }
            else {
                return keyIn.readBytesAtOffset(0, colLength);
            }
        }
        else { 
            short offset = valIn.readShortAtOffset(2 * colIndex);
            if (offset == -1) {
                return null;
            }
            if (colType == 0) {
                ...
                return valIn.readBytesAtOffset(offset, end - offset);
            }
        }
    }
